var generateHistory = function() {
    ...
    for (let cityCounter = 0;cityCounter < cityHistory?.length;cityCounter++) {
        ...
        $(".search-history-container").append(`<button id="CityBtn${cityCounter}" class="btn btn-dark btn-block">${city}</button>`);
...
var createQuery = function(city) {
    ...
    $.ajax({
        ...
        $.ajax ({
            ...
            $(".forecast-column").html("").append('<div class="jumbotron bg-light border border-dark rounded p-3 mt-2 todays-forecast"></div>');
            ...
            uvExtendedData?.daily?.map((day, index) => {
                if (index > 0 && index < 6) {
                    $(".card-deck").append(
                        `<div class="card bg-success px-2 text-white" id="${'card' + index}">
